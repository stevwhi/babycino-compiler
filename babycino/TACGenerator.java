...
public class TACGenerator extends MiniJavaBaseVisitor<TACBlock> {
    ...
    public TACBlock visitStmtBlock(MiniJavaParser.StmtBlockContext ctx) {
    	...
    	@Override
	public TACBlock visitExpEqual(MiniJavaParser.ExpEqualContext ctx) {
    	TACBlock result = new TACBlock();
    	TACBlock left = this.visit(ctx.expression(0));
    	TACBlock right = this.visit(ctx.expression(1));
	...
	String res = this.genreg();
    	...
    	result.add(TACOp.binop(temp, left.getResult(), right.getResult(), TACOp.binopToCode("-")));
